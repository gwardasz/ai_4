...
const STATE_FILE = path.join(taskRoot, '.langfuse-prompt-state.json')
...
const collectPromptSources = async (): Promise<PromptSource[]> => {
  const { buildFirmwareSystemPrompt } = await import('../../agent/prompt.js')

  return [
    {
      name: 'agents/firmware',
      content: buildFirmwareSystemPrompt(),
      tags: ['agent-template', 'firmware'],
    },
  ]
}
