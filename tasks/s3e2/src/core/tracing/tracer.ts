...
export const withAgent = async <T>(params: AgentParams, fn: () => Promise<T>): Promise<T> => {
  const contextName = params.contextName ?? params.name

  if (!isTracingActive()) {
    return withAgentContext(contextName, params.agentId, fn)
  }
  ...
  return startActiveObservation(
    ...
    async (span: LangfuseAgent) => {
      ...
      return withAgentContext(contextName, params.agentId, async () => {
        try {
          const result = await fn()
