...
export const buildVerifyPayload = (confirmation: string) => ({
...
export const submitConfirmation = async (
  ...
  const payload = buildVerifyPayload(confirmation)

  log.info('verify.request', { confirmation: `${confirmation.slice(0, 12)}...` })
  ...
  for (;;) {
    ...
    try {
      ...
      log.error('verify.network', { message })
