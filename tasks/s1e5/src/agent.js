...
// Wykonuje wywolania narzedzi sekwencyjnie (przy limitach API nie zalewamy serwera rownolegle).
...
/**
 * Jednorazowa petla agenta: rusza od neutralnego sygnalu startowego, iteruje az model przestanie wolac narzedzia.
...
export const run = async ({ model, tools, handlers, instructions }, log = noopLogger) => {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (flag) {
      ...
      return { reply: `Route X-01 activated. Flag captured: ${flag}`, history: conversation, flag };
    }

    // Deterministyczne przerwanie (np. otwarty circuit breaker) - nie czekamy na decyzje LLM.
    if (aborted) {
